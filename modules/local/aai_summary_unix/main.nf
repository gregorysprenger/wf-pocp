process AAI_SUMMARY_UNIX {
    ...
    path("Num_Proteins_Per_Proteome.tsv")
    ...
    msg "INFO: Summarizing each comparison to Summary.AAI.tsv"

    # Summarize AAI values
    echo -n '' > Summary.AAI.tsv
    for f in !{stats}; do
        ...
        echo -e "$S1\t$S2\t$FRAG\t$MEAN\t$STDEV\t$F1\t$M1\t$D1\t$F2\t$M2\t$D2" >> Summary.AAI.tsv
    ...
    sed -i "1i ${A}${B}${B}" Summary.AAI.tsv
