...
def parseArgs():
    parser = ArgumentParser(
        description="Computes the average amino acid " "identity (AAI) between two protein sets", add_help=False
    )
    ...
    req.add_argument(
        ...
        required=True,
        metavar="FILE",
        help="first input FastA or GenBank file " "(optionally gunzip compressed)",
    )
    req.add_argument(
        "-2",
        "--set2",
        required=True,
        metavar="FILE",
        help="second input FastA or GenBank file " "(optionally gunzip compressed)",
    )
    opt = parser.add_argument_group("Optional")
    opt.add_argument(
        "-c", "--cpus", type=require_int_nonnegative, metavar="INT", default="0", help="number of CPUs [all]"
    )
    opt.add_argument(
        "-f", "--fraction", type=float, metavar="FLOAT", default=70.0, help="minimum alignment length percentage [70.0]"
    )
    opt.add_argument("-h", "--help", action="help", help="show this help message and exit")
    opt.add_argument(
        "-i",
        "--identity",
        type=require_float_0_to_100,
        metavar="FLOAT",
        default=30.0,
        help="minimum percent identity [30.0]",
    )
    opt.add_argument(
        "-l",
        "--length",
        ...
        metavar="INT",
        default=0,
        help="minimum alignment character length (sum of all " "aligned segments and all gaps) [0]",
    )
    opt.add_argument("-o", "--outpath", metavar="PATH", default=None, help="output directory [AAI--<date>_<time>]")
    opt.add_argument(
        "-s", "--bitscore", type=float, metavar="FLOAT", default=0.0, help="minimum alignment Bit score [0.0]"
    )
    opt.add_argument(
        "--aligner", choices=["blastp", "diamond"], default="blastp", help="local protein aligner [blastp]"
    )
    opt.add_argument(
        "--decimal-places",
        ...
        metavar="INT",
        default=3,
        help="precision decimal points to round " "AAI values [3]",
    )
    opt.add_argument(
        "--max-ACGT",
        type=require_float_0_to_1,
        metavar="FLOAT",
        default=0.9,
        help="input sequence sets above this "
    ...
    opt.add_argument(
        ...
        metavar="FLOAT",
        default=0,
        help="minimum two-way alignment fraction " "of each set [0.0]",
    )
    opt.add_argument(
        "--min-aln-len",
        type=require_int_nonnegative,
        metavar="INT",
        default=0,
        help="minimum two-way alignment length of " "each set [0]",
    )
    opt.add_argument(
        "--name1", type=str, metavar="STR", default=None, help="identifier used for set1 output data [basename set1]"
    )
    opt.add_argument(
        "--name2", type=str, metavar="STR", default=None, help="identifier used for set2 output data [basename set2]"
    )
    opt.add_argument(
        "--refilter",
        default=False,
        ...
        for feat in rec.features:
            if feat.type == "CDS" and "translation" in feat.qualifiers:
                if len(feat.qualifiers["translation"]) == 1:
                    new_rec = SeqRecord(
                        Seq(feat.qualifiers["translation"][0], IUPAC.protein),
                        id=feat.qualifiers["locus_tag"][0],
                        description=rec.id,
                    )
...
def verify_protein_fasta_is_not_nucleotides(infile, max_acgt):
    ...
    observed_acgt_fraction = 1.0 * (c["A"] + c["C"] + c["G"] + c["T"]) / total_length
    if observed_acgt_fraction > max_acgt:
        sys.stderr.write(
            "ERROR: {} exceeds {} maximum allowed A, C, G, T "
            "composition. If {} is truly amino acids and not nucleotides, "
            "adjust the --max-ACGT value to compute AAI.\n".format(observed_acgt_fraction, max_acgt, infile)
        )
        sys.exit(1)
...
def main():
    ...
    if not opts.refilter:
        ...
        for s1, s2, sb1, sb2 in [(set1, set2, b1, b2), (set2, set1, b2, b1)]:
            ...
            if opts.aligner == "blastp":
                c1 = ["makeblastdb", "-in", s1, "-out", ref_db, "-dbtype", "prot"]
                c2 = [
                    "blastp",
                    "-db",
                    ref_db,
                ...
                c2 = [
                    ...
                    " qend sstart send evalue bitscore qcovhsp gaps"
                ).split()
            for cmd in (c1, c2):
    ...
    with open(set1qry_base + ".tab") as dat, open(set1qry_base + ".filt.tab", "w") as passed:
        for line in dat:
            tot[0] += 1
            l = line.split("\t")
            if (
                ...
                passed.write(line)
    with open(set2qry_base + ".tab") as dat, open(set2qry_base + ".filt.tab", "w") as passed, open(
        set2qry_base + ".filt.two-way.tab", "w"
    ) as passed2:
        for line in dat:
            tot[1] += 1
            l = line.split("\t")
    ...
    if not opts.refilter:
        set1_alnfrc = sum(filt[0]) / 1.0 / psize[0]
        set2_alnfrc = sum(filt[1]) / 1.0 / psize[1]
        bidirectional_alnfrc = (sum(filt[2]) / 1.0 / psize[0], sum(filt[2]) / 1.0 / psize[1])
        aln_frc = (set1_alnfrc, set2_alnfrc, bidirectional_alnfrc)
        tot = count_fasta_records(set1), count_fasta_records(set2)
    else:
        sys.stderr.write(
            "WARNING: Total protein counts, which are the "
    ...
    if opts.min_aln_len > sum(filt[2]):
        sys.stderr.write(
            "ERROR: {} bi-directional (two-way) gapless "
            "alignment length is less than {}\n".format(sum(filt[2]), opts.min_aln_len)
        )
        sys.exit(1)
    ...
    # Write AAI data
    with open(os.path.join(outpath, "aai." + b1 + "," + b2 + ".stats.tab"), "w") as ofh:
        ofh.write("Query_Sample(s)\tFiltered_Proteins\tAAI\tStDev\t" "Gapless_Aln\tInput_Size\tAln_Fraction\n")
        ofh.write(
            "{}\t{}/{}\t{:.{p}f}%\t{:.{p}f}%\t{}\t{}\t{:.{p}f}\n".format(
                b1, len(filt[0]), tot[0], mean(aai[0]), std(aai[0]), sum(filt[0]), psize[0], aln_frc[0], p=dec_pts
            )
        )
        ofh.write(
            "{}\t{}/{}\t{:.{p}f}%\t{:.{p}f}%\t{}\t{}\t{:.{p}f}\n".format(
                b2, len(filt[1]), tot[1], mean(aai[1]), std(aai[1]), sum(filt[1]), psize[1], aln_frc[1], p=dec_pts
            )
        )
        ofh.write(
            "{},{}\t{}/{},{}/{}\t{:.{p}f}%\t{:.{p}f}%\t{},{}\t{},{}\t"
